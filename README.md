...
그래서 다음 순서로 처리하도록 바꿨습니다.

- 아이템과 OCR 수량을 모두 찾으면 `found`
- 수량이 없으면 해당 셀의 수량 영역만 잘라 Upstage OCR로 다시 확인
- crop에서 하나의 숫자만 반환되면 그 값을 채택
- OCR도 빈 응답이면 로컬 숫자 glyph 템플릿과 비교
- 최고 점수와 2위 점수의 차이가 충분할 때만 glyph 값을 채택
- 숫자가 없거나 여러 값이 섞이면 `needs_review`
- 높은 템플릿 점수만으로 수량을 추측하지 않음

기존 `test1-1` 화면에 이 단계를 적용한 결과, 누락됐던 수량 `1`을 실제
OCR 응답으로 확인해 `found`로 바꿀 수 있었습니다. 이미 읽힌 `5`, `111`,
`20`은 재인식하거나 덮어쓰지 않았습니다.

추가한 `960x540` 화면에서는 선명한 `4` crop도 OCR과 Document Parse가
모두 빈 응답을 반환했습니다. 숫자 glyph를 배경과 분리해 정규화한 뒤 로컬
템플릿과 비교하자 서로 다른 캐릭터 4개 화면에서 모두 `4`로 분류됐습니다.
...
이 이미지는 개선 전 최초 OCR 결과입니다. 1번 아이템 아래에는 실제로 `1`이 보이지만 OCR 응답에는 해당 숫자의
bounding box가 없었습니다. 화면을 보고 값을 채워 넣는 대신 빈 수량과
`needs_review`를 반환했습니다. 이후 수량 영역 재인식에서는 `1` 하나만
반환되어 최종 결과가 `quantity: 1`, `status: found`로 바뀌었습니다.
...
| 신규 캐릭터 4명, `960x540` 화면 12장 | 캐릭터 ID 4개와 목표 수량 40개 모두 확정 |
...
여러 화면의 결과를 캐릭터 ID로 합친 뒤,
**한 캐릭터의 한 번의 캡처 세션을 한 행으로 기록**합니다.
...
`세션 ID + 캐릭터 ID`가 이미 있으면 해당 행을 갱신하고, 없을 때만 새 행을
추가합니다. Google 인증 없이 wide CSV만 만들 수도 있으며, Sheets 연동은
optional dependency로 분리했습니다.
...
새 화면에서 템플릿별 최고 점수와 좌표를 확인합니다.

```bash
rpg-progress-ocr-logger calibrate examples/screenshot.png \
  --templates local_templates \
  --profile scanner-profile.json
```

여러 스캔 결과를 캐릭터별 한 행의 CSV로 합칩니다.

```bash
rpg-progress-ocr-logger export-wide local_outputs/scan-*.json \
  --captured-at 2026-06-27T18:00:00+09:00 \
  --session-id 20260627-evening \
  --out local_outputs/inventory-wide.csv
```

Google Sheets에 같은 키의 행을 갱신하거나 새 행을 추가합니다.

```bash
python -m pip install -e ".[sheets]"
rpg-progress-ocr-logger export-sheets local_outputs/scan-*.json \
  --captured-at 2026-06-27T18:00:00+09:00 \
  --session-id 20260627-evening \
...
- 새 테스트 이미지에서 수량 crop 범위와 프로필 임계값 재검증
- Google Sheets 실제 계정에서의 통합 검증
